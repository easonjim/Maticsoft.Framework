...
$(window).load(function() {
    var $iconElement = $("[id$='ImageIcon']");
    var $imagedrag = $("[id$='ImageDrag']");
    ...
    //初始化默认值
    $("[id$='txt_width']").val(currentWidth);
    $("[id$='txt_height']").val(currentHeight);
    $("[id$='txt_top']").val(0-dragtop);
    $("[id$='txt_left']").val(0-dragleft);
    $("[id$='txt_Zoom']").val(scaleFactor);
    ...
    $imagedrag.draggable(
    {
        ...
        drag: function(e, ui) {
            ...
            $("[id$='txt_left']").val(0 - parseInt(self.position.left));
            $("[id$='txt_top']").val(0 - parseInt(self.position.top));
        }
    }
    );
    //设置图片可拖拽，并且拖拽一张图片时，同时移动另外一张图片
    $iconElement.draggable(
    {
        ...
        drag: function(e, ui) {
            var self = $(this).data("draggable");
            var drop_img = $("[id$='ImageDrag']");
            var top = drop_img.css("top").replace(/px/, ""); //取出截取框到顶部的距离
            var left = drop_img.css("left").replace(/px/, ""); //取出截取框到左边的距离
            drop_img.css({ left: (parseInt(self.position.left) - LEFT_EDGE) + "px", top: (parseInt(self.position.top) - TOP_EDGE) + "px" }); //同时移动内部的图片
            $("[id$='txt_left']").val(0 - (parseInt(self.position.left) - LEFT_EDGE));
            $("[id$='txt_top']").val(0 - (parseInt(self.position.top) - TOP_EDGE));
        }
