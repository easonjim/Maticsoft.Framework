...
{
    public partial class OffLineTeacherInfo : PageBaseUser
    {
        ...
        {
            Response.Redirect("OrganizeCourse.aspx?courseId=" + CourseId);
        }
        ...
        private void SaveInfo(bool type)
        {
            ...
            if (UserBll.EditUesInfo(TrueName, TeachDec, CurrentUser.UserID, this.txtDepartmentName.Value))
            {
                ...
                if (type)
                {
                    Response.Redirect("OffLineDocu.aspx?CourseId=" + CourseId);
                }
                else
                {
